...
describe('RadioGroup', () => {
    ...
    it('applies tones', () => {
        expect(
            render(<RadioGroup tone="danger" items={items} />)
                .querySelector('[role="radiogroup"]')
                ?.getAttribute('data-tone')
        ).toBe('danger')
        expect(
            render(<RadioGroup tone="neutral" items={items} />)
                .querySelector('[role="radiogroup"]')
                ?.getAttribute('data-tone')
        ).toBe('neutral')
    })
    ...
    it('exposes disabled state on the group', () => {
        const view = render(<RadioGroup items={items} disabled />)
        expect(view.querySelector('[role="radiogroup"]')?.getAttribute('aria-disabled')).toBe(
            'true'
        )
    })
    ...
    it.each(['sm', 'md', 'lg'] as const)('exposes the %s density hook', (size) => {
        ...
        const view = render(
            <RadioGroup
                items={[
                    {
                        value: 'long',
                        label: 'ExtraordinarilyLongLabelWithoutSpaces',
                        description: 'First line\nSecond line',
                    },
                ]}
            />
        )
        expect(view.querySelector('.mr-radio__label')?.textContent).toBe(
            'ExtraordinarilyLongLabelWithoutSpaces'
        )
        expect(view.querySelector('.mr-radio__description')?.textContent).toContain('Second line')
    })
})
