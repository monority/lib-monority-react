...
export const DateRangePicker = forwardRef<HTMLDivElement, DateRangePickerProps>(
  function DateRangePicker({ size, className, fromLabel = 'From', toLabel = 'To', fromProps, toProps }, ref) {
    return (
      <div ref={ref} className={cn('mr-date-range-picker', className)} data-size={size ?? 'md'}>
        <DatePicker label={fromLabel} size={size} {...fromProps} />
        <span className="mr-date-range-picker__separator" aria-hidden="true">→</span>
        <DatePicker label={toLabel} size={size} {...toProps} />
      </div>
    )
  },
)
