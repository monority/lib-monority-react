...
export const DateRangePicker = forwardRef<HTMLDivElement, DateRangePickerProps>(
    function DateRangePicker(
        ...
        const [fromValue, setFromValue] = useState<Date | null>(() =>
            parseDate(fromProps?.value ?? fromProps?.defaultValue)
        )
        const [toValue, setToValue] = useState<Date | null>(() =>
            parseDate(toProps?.value ?? toProps?.defaultValue)
        )
        ...
        const invalidRange = Boolean(
            resolvedFrom && resolvedTo && isOnOrBefore(resolvedTo, resolvedFrom)
        )
        ...
        const handleFromChange = useCallback(
            (date: Date | null) => {
                ...
                if (date && currentTo && isOnOrBefore(currentTo, date)) {
                    normalizedRangeRef.current = `${date.getTime()}:${currentTo.getTime()}`
        ...
        const toDisabledDates = resolvedFrom
            ? (date: Date) =>
                  isOnOrBefore(date, resolvedFrom) ||
                  (Array.isArray(originalDisabledDates)
                      ? originalDisabledDates.some(
                            (disabledDate) =>
        ...
        return (
            <div
                ref={ref}
                className={cn('mr-date-range-picker', className)}
                data-size={size ?? 'md'}
            >
                <DatePicker
                    {...fromProps}
                ...
                />
                <span className="mr-date-range-picker__separator" aria-hidden="true">
                    →
                </span>
                <DatePicker
                    {...toProps}
