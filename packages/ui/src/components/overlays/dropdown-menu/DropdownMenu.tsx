...
type DropdownAlign = 'start' | 'center' | 'end'; type DropdownSide = 'top' | 'bottom' | 'left' | 'right'
interface DropdownMenuItem { value: string; label: string; type?: 'item' | 'separator'; disabled?: boolean; danger?: boolean; onSelect?: (value: string) => void }
interface DropdownMenuProps { trigger: React.ReactNode | string; items?: DropdownMenuItem[]; open?: boolean; defaultOpen?: boolean; onOpenChange?: (open: boolean) => void; align?: DropdownAlign; side?: DropdownSide; className?: string; contentClassName?: string }

function isActionableItem(item: DropdownMenuItem) { return item.type !== 'separator' && !item.disabled }

export function DropdownMenu({ trigger, items = [], open, defaultOpen = false, onOpenChange, align = 'end', side = 'bottom', className, contentClassName }: DropdownMenuProps) {
  ...
  const updatePosition = useCallback(() => {
    const el = triggerElementRef.current ?? rootRef.current?.querySelector('[data-ui-dropdown-trigger="true"]')
  ...
  useEffect(() => {
    ...
    function kd(e: KeyboardEvent) { if (e.key === 'Escape') { setOpenState(false); (rootRef.current?.querySelector('[data-ui-dropdown-trigger="true"] button, [data-ui-dropdown-trigger="true"] a, [data-ui-dropdown-trigger="true"] [tabindex]') as HTMLElement)?.focus() } }
  ...
  return <div ref={rootRef} className={cn('ui-dropdown', className)}>
    {typeof trigger === 'string' ? <button type="button" className="ui-dropdown__trigger" data-ui-dropdown-trigger="true" aria-expanded={isOpen} aria-controls={`${instanceId}-content`} aria-haspopup="menu" onClick={(e) => { triggerElementRef.current = e.currentTarget; updatePosition(); setOpenState(!isOpen) }} onKeyDown={(e) => { if (e.key === 'ArrowDown' || e.key === 'Enter' || e.key === ' ') { e.preventDefault(); updatePosition(); setOpenState(true) } }}>{trigger}</button>
    : <span className="ui-dropdown__anchor" data-ui-dropdown-trigger="true" aria-expanded={isOpen} aria-controls={`${instanceId}-content`} aria-haspopup="menu" onClick={(e) => { triggerElementRef.current = (e.target as HTMLElement).closest('button, a, [tabindex]') ?? e.currentTarget; updatePosition(); setOpenState(!isOpen) }} onKeyDown={(e) => { if (e.key === 'ArrowDown' || e.key === 'Enter' || e.key === ' ') { e.preventDefault(); updatePosition(); setOpenState(true) } }}>{trigger}</span>}
    {isOpen && portalTarget ? createPortal(<div ref={contentRef} id={`${instanceId}-content`} className={cn('ui-dropdown__content', `ui-dropdown__content--${side}`, `ui-dropdown__content--${align}`, contentClassName)} role="menu" aria-orientation="vertical" style={{ top: `${position.top}px`, left: `${position.left}px` }} onKeyDown={(e) => { if (e.key === 'ArrowDown') { e.preventDefault(); focusItem(1) } if (e.key === 'ArrowUp') { e.preventDefault(); focusItem(-1) } if (e.key === 'Home') { e.preventDefault(); focusItem(1, actionableItems[0]?.value) } if (e.key === 'End') { e.preventDefault(); focusItem(-1, actionableItems[actionableItems.length - 1]?.value) } }}>
      {items.map((item, index) => item.type === 'separator' ? <div key={`${instanceId}-sep-${index}`} className="ui-dropdown__separator" role="separator" /> : <button key={item.value} type="button" role="menuitem" data-value={item.value} className={cn('ui-dropdown__item', item.danger && 'is-danger')} disabled={item.disabled} onClick={() => { item.onSelect?.(item.value); setOpenState(false) }}>{item.label}</button>)}
