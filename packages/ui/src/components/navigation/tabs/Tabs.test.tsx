...
describe('Tabs', () => {
  ...
  it('maps disabled and fullWidth states', () => {
    const view = render(<Tabs items={items} value="tab1" onChange={() => {}} disabled fullWidth />)
    const tablist = view.querySelector('[role="tablist"]')
  ...
  it('fires onChange when a tab is clicked', () => {
    let selected = 'tab1'
    const handleChange = (value: string) => { selected = value }
    const view = render(<Tabs items={items} value="tab1" onChange={handleChange} />)
    act(() => { (view.querySelectorAll('[role="tab"]')[2] as HTMLButtonElement).click() })
  ...
  it('supports keyboard navigation with ArrowRight', () => {
    let selected = 'tab1'
    const view = render(<Tabs items={items} value="tab1" onChange={(v) => { selected = v }} />)
    act(() => {
  ...
  it('allows overriding aria-label', () => {
    const view = render(<Tabs items={items} value="tab1" onChange={() => {}} aria-label="Custom" />)
  ...
  it('navigates to first tab with Home', () => {
    let selected = 'tab3'
    const view = render(<Tabs items={items} value="tab3" onChange={(v) => { selected = v }} />)
    act(() => {
