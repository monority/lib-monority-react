...
export function AsyncStateNoticeCustomMessageExample() {
  return (
    <AsyncStateNotice
      isLoading
      loadingMessage="Fetching user profile..."
    />
  )
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div style={{ display: 'flex', gap: '0.5rem' }}>
        <Button size="sm" onClick={() => setState('loading')}>Load</Button>
        <Button size="sm" variant="danger" onClick={() => setState('error')}>Error</Button>
